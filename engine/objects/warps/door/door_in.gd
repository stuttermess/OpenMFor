...
func _physics_process(delta: float)->void :
    ...
    if player.is_on_floor() && !_on_warp && player.warp == Player.Warp.NONE && !(&"holding" in player.suit.extra_vars && player.suit.extra_vars.is_holding) && player.global_position.y <= pos_player.global_position.y + 8:
