...
func _shoot(index: int)->void :
    if cannon_ball:
        var cball: CannonBall = NodeCreator.prepare_2d(cannon_ball, self).bind_global_transform(
    ...
    if cannon_explosion_effect:
        var eff: = NodeCreator.prepare_2d(cannon_explosion_effect, self).create_2d().get_node()
