...
func _shoot_bullet(pos: Marker2D, dir: int)->void :
    ...
    NodeCreator.prepare_ins_2d(bullet_bill, self).create_2d(false).call_method(configure_bullet)
    ...
    NodeCreator.prepare_2d(explosion, self).create_2d(false).call_method(configure_eff)
